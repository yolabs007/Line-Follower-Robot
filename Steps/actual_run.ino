/*
1.  Start the power button 
2. A short quick press will start the dry run 
3. shortest path will be saved in memort 
4. restart the power button 
5 and  press te push button actual run will start 
6.  to erase memory press the push button for 2 Sec */

// ===== Grid Solver + NVS Shortest-Path Record/Replay =====
// - START button (D5→GND). Hold 2s at boot to CLEAR saved path.
// - END box (>=7/8 white) → simplify + save to NVS, stop, LED on D2.
// - Next run: if a saved path exists, REPLAY it (ignore detection).
// - L/R turns finish like U-turn (center capture). TURN_SPEED=100.
...
void clearSavedPath(){
  ...
  SerialBT.println("NVS cleared.");
}

// ----- Start/clear button -----
void waitForStartOrClear(){
  Serial.println("Hold START (D5) for ~2s at boot to CLEAR path. Tap to start run.");
  SerialBT.println("Hold START (D5) for ~2s to CLEAR. Tap to start.");
  unsigned long t0 = millis();
  // Wait up to ~2.5s to detect a long press
  ...
  // Now wait for a normal press-release to start
  Serial.println("Press START to begin…");
  SerialBT.println("Press START to begin…");
...
void setup(){
  ...
  if(replayMode){
    ...
    SerialBT.printf("Saved shortest path: %s\n", savedPath.c_str());
  } else {
    Serial.println("No saved path. This run will RECORD raw path.");
    SerialBT.println("No saved path. Recording raw path…");
  }

  waitForStartOrClear(); // allow clearing before starting

  // If cleared during the dialog, refresh replay flag
  savedPath = readSavedPath();
  replayMode = (savedPath.length() > 0);
  pathIndex = 0;

  Serial.println(replayMode ? "Mode: REPLAY" : "Mode: EXPLORE");
  SerialBT.println(replayMode ? "Mode: REPLAY" : "Mode: EXPLORE");
...
void loop(){
  ...
  // -------------------- REPLAY MODE --------------------
  if(replayMode){
    // In replay: follow corridors normally, but at each junction we execute the next move
    if(state==FOLLOW){
      // trigger when detection says "there is a junction" (or dead-end)
    ...
    if(state==NUDGE){
      ...
      if(millis()-stateStart >= NUDGE_MS){
        ...
        if (whiteCount >= END_WHITE_MIN || allWhite()){
          ...
          SerialBT.println("END detected (REPLAY) → HALT");
          enter(HALT);
          return;
        }

        // Execute the next move in savedPath
        if(pathIndex >= (int)savedPath.length()){
          // Path finished, stop
          motorsAllLow();
          digitalWrite(RUN_LED, HIGH);
          Serial.println("Path finished — HALT.");
          SerialBT.println("Path finished — HALT.");
          enter(HALT);
          return;
        }
    ...
    // re-use the same TURN/STRAIGHT/RECENTER states below
  }
    ...
    if(state==NUDGE){
      ...
      if(millis()-stateStart >= NUDGE_MS){
        ...
        if (whiteCount >= END_WHITE_MIN || allWhite()){
          ...
          writeSavedPath(shortest);
          motorsAllLow();
          digitalWrite(RUN_LED, HIGH);
          printSnap("END_BOX");
          Serial.printf("END detected → stored shortest: %s\n", shortest.c_str());
          SerialBT.printf("END detected → stored shortest: %s\n", shortest.c_str());
          enter(HALT);
          return;
        }
        ...
        if (sawLeft && hasStraight) { rawPath += 'L'; Serial.println("Action: LEFT");  SerialBT.println("Action: LEFT");  enter(TURN_L); }
        else if (!sawLeft && sawRight && hasStraight) { rawPath += 'S'; Serial.println("Action: STRAIGHT"); SerialBT.println("Action: STRAIGHT"); enter(GO_STRAIGHT); }
        else if (sawLeft && !hasStraight) { rawPath += 'L'; Serial.println("Action: LEFT");  SerialBT.println("Action: LEFT");  enter(TURN_L); }
        else if (sawRight && !hasStraight) { rawPath += 'R'; Serial.println("Action: RIGHT"); SerialBT.println("Action: RIGHT"); enter(TURN_R); }
        else if (sawLeft && sawRight && !hasStraight) { rawPath += 'L'; Serial.println("Action: LEFT"); SerialBT.println("Action: LEFT"); enter(TURN_L); }
        else if (!sawLeft && !sawRight && hasStraight) { rawPath += 'S'; Serial.println("Action: STRAIGHT"); SerialBT.println("Action: STRAIGHT"); enter(GO_STRAIGHT); }
        else { rawPath += 'U'; Serial.println("Action: U-TURN"); SerialBT.println("Action: U-TURN"); enter(UTURN); }
      }
      return;
    }
  }
